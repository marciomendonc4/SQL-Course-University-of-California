...
SELECT *
FROM fireIncidents
WHERE `Incident Date` IN ('09/27/2016', '04/04/2016')
LIMIT 10;
...
FROM fireIncidents
WHERE `Incident Date` IN ('09/27/2016', '04/04/2016') AND `Station Area` > 20
LIMIT 10;
...
-- TODO
SELECT `Ignition Cause`, COUNT(1) AS Amount
FROM fireIncidents
WHERE `Ignition Cause` LIKE '4 act%'
...
-- TODO
DROP TABLE IF EXISTS fireCall;

CREATE TABLE IF NOT EXISTS fireCall
USING CSV
OPTIONS(
  header "true",
